...
def create_calculator_tool(logger: ToolLogger):
    ...
    def calculator(expression: str) -> str:
        ...
        Use this tool for ALL numeric calculations, even very simple ones like 2+2.
        Supported characters: digits, spaces, decimal points, parentheses, and + - * / operators.
        """
        # 1. Validate the expression for safety
        allowed_pattern = r"^[0-9\.\+\-\*\/\(\)\s]+$"
        if not re.fullmatch(allowed_pattern, expression):
            error_msg = (
                "Invalid expression. Only digits, spaces, parentheses, and "
        ...
        try:
            # 2. Evaluate the expression in a restricted environment
            result = eval(expression, {"__builtins__": {}}, {})

            # 3. Format the result as a user-friendly string
            result_str = f"{expression} = {result}"

            # 4. Log successful tool usage
            logger.log_tool_use(
                "calculator",
                {"expression": expression},
                {"result": result}
            ...
            # 5. Handle and log errors gracefully
            error_msg = f"Error while evaluating expression '{expression}': {e}"
            logger.log_tool_use(
                "calculator",
                {"expression": expression},
                {"error": error_msg}
